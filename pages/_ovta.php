<center> <div class="tegname"><h2>Загон Коз</h2></div><br> </center>
<?php
...
if(!isset($_SESSION['zagon']['tab']['3'])) $_SESSION['zagon']['tab']['3'] = 1;


//$sql_p = mysql_query("SELECT * FROM `tb_kura` WHERE `num` = '1' AND `username` = '$login'") or die(mysql_error());

$row = $db->getRow("SELECT `price_ovta` FROM `tb_price`");

?>
<link rel="stylesheet" type="text/css" href="/css/pole.css" />
<a class="tooltip" href="javascript:void(0)">Загон Коз [?]<span class="custom help" style="width: 450px;"><em>Информация</em>В каждый загон можно посадить 9 коз.<br>
	Животных надо кормить каждые 24 часов. После того, как Вы покормили животное и прошло 24 часов, Вы получите две единицу продукции, которую надо собрать.<br>
	1 коза дает 60 молока, после чего навсегда покидает игру.<br>
	Каждое кормление забирает 6 ед. энергии и дает 3 ед. опыта.<br>
	Каждый сбор продукта забирает 2 ед. энергии и дает 2 ед. опыта.<br>
</span></a>
<br><br>
<link rel="stylesheet" type="text/css" href="/css/pole1.css" />
<p style="border: 3px solid #0E82A7;border-radius: 8px;padding: 2%;">На полях Вы можете купить загоны коз для животных.<br>
	
	<br>У Вас в наличии корм: <span id="span_korm"><?=$us_data['jachmen']; ?></span> шт.<br>
	У Вас в наличии млока: <span id="span_product"><?=$us_data['m_o']; ?></span> шт.<br>
	У Вас коз для посадки: <span id="span_animals"><?=$us_data['ovta']; ?></span> шт.<br>
	<br>Цена покупки загона коз: <?=$row['price_ovta']; ?> руб.<br>
	Друг-реферер получает 10% от стоимости загона коз.</p>
...
<span>Собрать / покормить</span>
	...
	<ul>
		...
		<li class="active" ><a href="/ovta"><span>Загон Коз</span></a></li>
...
$_SESSION['zagon']['tab']['3'] = count_page_zagon($_GET['id'], '3');

$q = $db->query("SELECT * FROM tb_zagon_id WHERE zagon_id = ?i AND `type` = ?i AND login = ?s", $id_kurat, $type_zagon, $login);
if($db->numRows($q) == 0){
echo '<center><font color="red">Это не ваша овчарня</font></center>';
Header("Refresh: 3, /ovta");
exit();
}else{
$id_kur = 1;
?>
...
<div class="answer_zagon" style="display: block;">
				<a href="/ovta"><b class="close">X</b></a><br>
...
$koll = $kol_pole / 9;
	...
	?>
	<li <?php if($i == $_SESSION['zagon']['tab']['3']) echo 'class="active"'; ?>><a href="#pole<?=$i; ?>"><span><?=$i; ?></span></a></li>
	<? } ?>
...
?>
<div id="pole<?=$k; ?>" class="tab_content" style="display:<?if ($k==$_SESSION['zagon']['tab']['3']) print "block;"; else print"none;";?>">
...
<script> $('#pole<?php echo $_SESSION['zagon']['tab']['3']; ?>').css({'display':'block'}); </script>
<?php $_SESSION['zagon']['tab']['3'] = 1; ?>
</div></div>
