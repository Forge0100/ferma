<center> <div class="tegname"><h2>Загон Кур.</h2></div><br> </center>
<?php
...
if(!isset($_SESSION['zagon']['tab']['1'])) $_SESSION['zagon']['tab']['1'] = 1;

$sql_p = $db->query("SELECT * FROM `tb_zagon` WHERE `num` = '1' AND `type` = 1 AND `login` = ?s", $login);
if($db->numRows($sql_p) == 0) {
$db->query("INSERT INTO `tb_zagon` (`user_id`, `login`, `type`, `num`, `pay`) VALUES (?i, ?s, '1',  '1', '1') ", $usid, $login);
for($i = 1; $i <= 9; $i++) {
$db->query("INSERT INTO `tb_zagon_id` (`zagon_id`, `user_id`, `login`, `type`, `num`, `status`) VALUES ('1', ?i, ?s, '1', ?i, '0')", $usid, $login, $i);
}
}
$row = $db->getRow("SELECT `price_kuratnik` FROM `tb_price`");

?>
<link rel="stylesheet" type="text/css" href="/css/pole.css" />
<a class="tooltip" href="javascript:void(0)">Загон Кур [?]<span class="custom help" style="width: 450px;"><em>Информация</em>В каждый загоне можно посадить 9 кур.<br>
	Животных надо кормить каждые 24 часов. После того, как Вы покормили животное и прошло 24 часов, Вы получите две единици продукции, которую надо собрать.<br>
	1 кура дает 60 яиц, после чего навсегда покидает игру.<br>
	Каждое кормление забирает 4 ед. энергии и дает 2 ед. опыта.<br>
	Каждый сбор продукта забирает 4 ед. энергии и дает 2 ед. опыта.<br>
</span></a>

<br><br>
<link rel="stylesheet" type="text/css" href="/css/pole1.css" />
<p style="border: 3px solid #0E82A7;border-radius: 8px;padding: 2%;">На полях Вы можете купить загоны кур для животных.<br>
	
	<br>У Вас в наличии корм: <span id="span_korm"><?=$us_data['pshenica']; ?></span> шт.<br>
	У Вас в наличии яиц: <span id="span_product"><?=$us_data['yaco']; ?></span> шт.<br>
	У Вас кур для посадки: <span id="span_animals"><?=$us_data['kurita']; ?></span> шт.<br>
	<br>Цена покупки загона кур: <?=$row['price_kuratnik']; ?> руб.<br>
	Друг-реферер получает 10% от стоимости курятника.</p>
...
<span>Собрать / покормить</span>
	...
	<ul>
		...
		<li class="active" ><a href="/newproduct"><span>Загон Кур</span></a></li>
...
$_SESSION['zagon']['tab']['1'] = count_page_zagon($_GET['id'], '1');
...
if($db->numRows($q) == 0){
echo '<center><font color="red">Это не ваш загон кур</font></center>';
Header("Refresh: 3, /newproduct");
exit();
}else{
$id_kur = 1;
?>
...
<div class="answer_zagon" style="display: block;">
				<a href="/newproduct"><b class="close">X</b></a><br>
				<div id="loadA"><div id="loadB">
<span id="answer_span">
				<div id="poleinanswer">
			
				<div id="pole_<?=$id_kur;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur, $type_zagon) ; ?>" style="top: 15px; left: 148px; cursor: pointer;">
				
				<div  class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+1;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+1, $type_zagon) ; ?>" style="top: -43px; left: 74px; cursor: pointer;">
			
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+1;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+2;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+2, $type_zagon) ; ?>" style="top: -101px; left: 0px; cursor: pointer;">
				
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+2;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+3;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+3, $type_zagon) ; ?>" style="top: -247px; left: 227px; cursor: pointer;">
				
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+3;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+4;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+4, $type_zagon) ; ?>" style="top: -305px; left: 153px; cursor: pointer;">
				
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+4;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+5;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+5, $type_zagon) ; ?>" style="top: -362px; left: 79px; cursor: pointer;">
				
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+5;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+6;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+6, $type_zagon) ; ?>" style="top: -509px; left: 307px; cursor: pointer;">
				
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+6;?>', '<?=$type_zagon;?>');"></div></div>
				
				<div id="pole_<?=$id_kur+7;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+7, $type_zagon) ; ?>" style="top: -567px; left: 232px; cursor: pointer;">
			
				<div class="click_div" onclick="korm_kur('<?=$id_kurat;?>', '<?=$id_kur+7;?>', '<?=$type_zagon;?>');"></div></div>
			
				<div id="pole_<?=$id_kur+8;?>" class="pol_img   <?php echo kur_by($id_kurat, $login, $id_kur+8, $type_zagon) ; ?>" style="top: -624px; left: 158px; cursor: pointer;">
...
$koll = $kol_pole / 9;
	...
	?>
	<li <?php if($i == $_SESSION['zagon']['tab']['1']) echo 'class="active"'; ?>><a href="#pole<?=$i; ?>"><span><?=$i; ?></span></a></li>
	<?php } ?>
...
<div id="pole<?=$k; ?>" class="tab_content" style="display:<?php if ($k == $_SESSION['zagon']['tab']['1']) print "block;"; else print"none;";?>">
...
<?php } ?>
<script> $('#pole<?php echo $_SESSION['zagon']['tab']['1']; ?>').css({'display':'block'}); </script>
<?php $_SESSION['zagon']['tab']['1'] = 1; ?>
</div></div>
