<center> <div class="tegname"><h2>Загон Бычков</h2></div><br> </center>
<?php
...
if(!isset($_SESSION['zagon']['tab']['2'])) $_SESSION['zagon']['tab']['2'] = 1;

//$sql_p = mysql_query("SELECT * FROM `tb_kura` WHERE `num` = '1' AND `username` = '$login'") or die(mysql_error());
$row = $db->getRow("SELECT `price_svin` FROM `tb_price`");

?>
<link rel="stylesheet" type="text/css" href="/css/pole.css" />
<a class="tooltip" href="javascript:void(0)">Загон Бычков [?]<span class="custom help" style="width: 450px;"><em>Информация</em>В каждый загон можно посадить 9 бычков.<br>
	...
	1 бычок дает 60 мяса, после чего навсегда покидает игру.<br>
	Каждое кормление забирает 4 ед. энергии и дает 4 ед. опыта.<br>
	Каждый сбор продукта забирает 2 ед. энергии и дает 2 ед. опыта.<br>
</span></a>
<br><br>
<link rel="stylesheet" type="text/css" href="/css/pole1.css" />
<p style="border: 3px solid #0E82A7;border-radius: 8px;padding: 2%;">На полях Вы можете купить загоны бычков для животных.<br>
	
	<br>У Вас в наличии корм: <span id="span_korm"><?=$us_data['kukurudza']; ?></span> шт.<br>
	У Вас в наличии мяса: <span id="span_product"><?=$us_data['myaso']; ?></span> шт.<br>
	У Вас бычков для посадки: <span id="span_animals"><?=$us_data['svinya']; ?></span> шт.<br>
	<br>Цена покупки загона бычков: <?=$row['price_svin']; ?> руб.<br>
	Друг-реферер получает 10% от стоимости загона бычков.</p>
...
<span>Собрать / покормить</span>
	...
	<ul>
		...
		<li class="active" ><a href="/svin"><span>Загон Бычков</span></a></li>
...
$_SESSION['zagon']['tab']['2'] = count_page_zagon($_GET['id'], '2');

$q = $db->query("SELECT * FROM tb_zagon_id WHERE zagon_id = ?i AND `type` = ?i AND login = ?s", $id_kurat, $type_zagon, $login);
if($db->numRows($q) == 0){
echo '<center><font color="red">Это не ваш загон бычков</font></center>';
Header("Refresh: 3, /svin");
exit();
}else{
$id_kur = 1;
?>
...
<div class="answer_zagon" style="display: block;">
				<a href="/svin"><b class="close">X</b></a><br>
...
$koll = $kol_pole / 9;
	...
	?>
	<li <?php if($i == $_SESSION['zagon']['tab']['2']) echo 'class="active"'; ?>><a href="#pole<?=$i; ?>"><span><?=$i; ?></span></a></li>
	<? } ?>
...
?>
<div id="pole<?=$k; ?>" class="tab_content" style="margin-left: -3px;display:<?if ($k==$_SESSION['zagon']['tab']['2']) print "block;"; else print"none;";?>">
<div id="pole" unselectable="on">

<span style="font-size: 18px;position: absolute;color: #4756EA;margin-left: 13px;">Поле № (<?=$k; ?>)</span>
...
<script> $('#pole<?php echo $_SESSION['zagon']['tab']['2']; ?>').css({'display':'block'}); </script>
<?php $_SESSION['zagon']['tab']['2'] = 1; ?>
</div></div>
