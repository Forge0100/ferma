...
class db
{
	...
	// Модификатор private - чтобы нельзя было создать еще один объект
	    // конструкцией clone($db);
	    private function __clone(){}
	    // Модификатор private - чтобы нельзя было создать еще один объект
	    // конструкцией unserialize($db);
	...
	{	
		$this->LastQuery =  $this->rawQuery($this->prepareQuery(func_get_args()));
	 ...
	 */
	public function fetch($result = false,$mode=self::RESULT_ASSOC)
	{
		if ($result==false) 
	 ...
	 */
	public function FreeMemory($result = false)
	{
		if ($result==false) 
			$result = $this->LastQuery;
